...
5. 展示前检测本地图鉴文件是否存在；若不存在，改用在线地址 `https://i.ibb.co/d49N5FLB/style-list.png` 通过 Markdown 展示。
6. 若本地图片与在线图片均无法展示，回退到下方“15 种信息图风格完全对照索引表”供用户选择。
7. 若用户已经明确指定风格，跳过图鉴展示并直接进入后续流程。

---

### 提示词优先与非 Codex 环境提醒

1. 必须先生成并交付完整提示词，再判断是否调用生图工具；不得跳过提示词直接生成图片。
2. 若当前不在 Codex 环境中，必须明确提醒用户：当前环境可能无法很好地绘制高信息密度信息图，请复制提示词到 ChatGPT 或 Gemini 中生成图片，以获得更好的效果。
